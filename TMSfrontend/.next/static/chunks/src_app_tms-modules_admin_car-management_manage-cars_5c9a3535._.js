...
(()=>{
    const e = new Error("Cannot find module '../api/carServices'");
...
function ManageCars() {
    ...
    const handleSearch = async (query)=>{
        try {
            setIsLoading(true);
            const { success, data, message } = await searchCars(query);
            if (success) {
                setFilteredCars(data);
            } else {
                throw new Error(message);
    ...
    const handleAddCar = async (newCar)=>{
        try {
            const { success, data, message } = await createCar(newCar);
            if (success) {
                setCars((prev)=>[
                        ...prev,
                        data
    ...
    const handleUpdateCar = async (updatedCar)=>{
        try {
            const { success, data, message } = await updateCar(updatedCar.id, updatedCar);
            if (success) {
